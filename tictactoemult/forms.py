...
class CreateAccountForm(forms.Form):
    username = forms.CharField(label='', max_length=30, widget=forms.TextInput(attrs={'placeholder': 'Username', 'class': 'create-account-text-input'}))
    ...
    password = forms.CharField(label='', max_length=32, widget=forms.TextInput(attrs={'placeholder': 'Password', 'class': 'create-account-text-input'}))
    description = forms.CharField(label='', required=False, max_length=300, widget=forms.Textarea(attrs={'placeholder': 'Description'}))
    checkbox = forms.BooleanField(label='By checking this checkbox, i agree to the terms and conditions')